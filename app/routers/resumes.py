...
from app.services.parser import parse_resume_from_blob, parse_zip_from_blob
...
async def upload_resume(resume: List[UploadFile] = File(...), request: Request = None):
    ...
    try:
        ...
        for uploaded_file in resume:
            ...
            # Parse directly from blob storage (no local persistence)
            if file_ext == ".zip":
                parsed_list = parse_zip_from_blob(blob_name, user_id=user_id)
                results.append({
                    "status": "success",
